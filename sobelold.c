...
//I FOUND THIS IMPLEMENTATION ONLINE
//LINK: https://github.com/fzehracetin/sobel-edge-detection-in-c/tree/main
...
int main(int argc, char **argv)
{
	...
	pgm image, out_image;
	char dir[200];
	//getting rid of this and are going to use command line arguments instead 
	//printf("Enter the file name: ");
	//scanf("%s", dir);
	read_pgm_file(argv[1], &image);
	padding(&image);
	...
	printf("\nGradient saved: %s \n", dir);
	//time(&program_end);
	program_end = omp_get_wtime() - program_start;
	printf("TIME SPENT:-----\n");
	printf("Rank %d: %.10f\n", rank, program_end);
